...
with st.sidebar:
    ...
    tagline=st.text_input("キャッチコピー","YDesign that ships.")
    meta_desc=st.text_area("eta description", "小さく速く出して、継続的に磨くためのデザインと実装。")
    ...
    st.divider()
    st.header("AI 生成認定")
    temperature=st.slider("多様性（temperature）",00.2, 1.4, 1.0, 0.1)
...
def replace_root_vars(css_text: str, new_vars_: Dict[str, str]) -> str:
    def repl(match:re.Match)->str:
        ...
        for ln in block.split(";"):
            if ":" in ln:
                ...
                key=k.strip()
                if k in new_vars_:
                    pairs.append(f"  {key}: {new_vars_[key]}" ); seen.add(key)
                else:
                    pairs.append(f"  {key}: {v}")
        for k,v in new_vars_.items():
            if k not in seen:pairs.append(f"  {k}: {v}")
        return ":root {" + ";".join(pairs) + "}"
    if not VAR_RE.search(css_text):
        head=":root {" + ";".join([f"  {k}: {v}" for k,v in new_vars_.items()]) + "}\n"
...
def data_url_from_file(file)->Tuple[str,str]:
    cdata=file.read()
...
def replace_placeholder_with_img(html: str, placeholder: str, data_url: str) -> str:
    cls=""
    m=re.search(r'class\s*=\s*"([^"]*img[^"]*)"', placeholder, re.I)
    if m:cls=m.group(1) 
    alt="image"
    m2=re.search(r'aria-label\s*=\s*"([^"]*?)"', placeholder, re.I)
